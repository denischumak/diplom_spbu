...
https://github.com/user-attachments/assets/6214d94b-9c92-485d-be34-6abce7897391
...
* [Итоговый прототип сенсорной перчатки](#итоговый-прототип-сенсорной-перчатки)
...
* Используется встроенный PGA (Gain = 8) для повышения разрешения сигнала;
* Аналоговые RC-фильтры и фильтрация питания минимизируют шум измерительных каналов;

### Сравнение встроенного и внешнего АЦП

<table width="100%">
  <tr>
    <td width="50%" align="center">
      <figure>
        <img src="docs/OUT_SS494B_CLEAN_TO_ESP32_100Hz.png" alt="Левый график" width="100%">
        <figcaption><b>Рис. 1.</b> Гистограмма для встроенного АЦП в ESP32-C3. </figcaption>
      </figure>
    </td>
    <td width="50%" align="center">
      <figure>
        <img src="docs/OUT_SS494B_TO_ADS1115_475SPS_100HZ_GAIN1_SE.png" alt="Правый график" width="100%">
        <figcaption><b>Рис. 2.</b> Гистограмма для АЦП ADS1115 (SE-вход, Gain = 1, 475 SPS). </figcaption>
      </figure>
    </td>
  </tr>
</table>

Размах шума снизился в 19,5 раз, а стандартное отклонение — более чем в 20 раз. Шум встроенного в ESP32-C3 АЦП по своему размаху превосходит полезный сигнал на выходе с датчика Холла (50-110 мВ).
...
* В параметрах встроенного в ADS1115 усилителя брался Gain = 8 (3200 тиков на 50 мВ), Data rate = 475 SPS, псевдодифф. режим с
вычитанием опорного напряжения, RC-фильтр со срезом на частоте ≈ 11 Гц;
...
<p align="center">
<b>Рис. 3.</b> Временные диаграммы сигналов датчиков Холла при одновременном сгибании пальцев.
</p>
...
## Итоговый прототип сенсорной перчатки
...
* Параметр β в фильтре Маджвика равен 0,1

---

## Датасет

* Для обучения и оценки модели был сформирован собственный датасет динамических жестов русского жестового языка;
* Запись данных выполнялась с использованием разработанной сенсорной перчатки;
* Каждый пример представляет собой многоканальный временной ряд, содержащий показания датчиков Холла, инерциального измерительного модуля и рассчитанные кватернионы ориентации кисти;
...
| Обучающая и валидационная выборка | 5 участников (1280 примеров) |
...
`Score = 1, 0 · IoU + 0, 6 · Recall − 0, 3 · Error_start − 0, 3 · Error_end`, 

где IoU — Intersection over Union, Error_start и Error_end — ошибки предсказания начала и конца жеста соответственно.
...
* 3 датчика Холла (аппаратно поддерживается пять, однако для данного датасета было достаточно и трех);
...
<p align="center">
<b>Рис. 8.</b> Схема одного остаточного блока.
</p>
...
<p align="center">
<b>Рис. 9.</b> График обучения модели с тремя остаточными блоками.
</p>
